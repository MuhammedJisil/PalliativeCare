...
app.get('/api/health-status/:patient_id', async (req, res) => {
  ...
  try {
    ...
    // Fetch the most recent health status
    const result = await client.query(
      'SELECT * FROM health_status WHERE patient_id = $1 ORDER BY note_date DESC LIMIT 1',
      [patientId]
    );

    // If no health status found, return default values
    if (result.rows.length === 0) {
      console.log(`No health status found for patient_id: ${patientId}`);
      return res.json({
        disease: '',
        medication: '',
        note: '',
    ...
    console.log(`Health status for patient_id ${patientId}:`, result.rows[0]);
    res.json(result.rows[0]);
  } catch (error) {
...
app.get('/api/medical-history/:patient_id', async (req, res) => {
  ...
  try {
    ...
    console.log(`Fetching medical history for patient_id: ${patientId}`);

    // Fetch all medical history records for the given patient ID
    const result = await client.query(
    ...
    if (result.rows.length === 0) {
      console.log(`No medical history found for patient_id: ${patientId}`);
      return res.json([]);
    }

    // Return all medical history records
    console.log(`Medical history for patient_id ${patientId}:`, result.rows);
    res.json(result.rows);
  } catch (error) {
